...
class Info():
    def __init__(self, game_info, state):
        ...
        self.flashing_coin = coin.FlashCoin(280, 35)
    ...
    def create_info_labels(self):
        ...
        self.create_label(self.score_text, '000000', 65, 35)
        self.create_label(self.coin_count_text, '*00', 300, 35)
        self.create_label(self.mario_label, 'MARIO', 75, 10)
        self.create_label(self.world_label, 'WORLD', 450, 10)
        self.create_label(self.best_score_label, 'BEST', 625, 10)
        score = ''
        with open("resources/score.txt", 'r') as file:
            score = file.read()
        self.create_label(self.best_score_text, '0' * (6 - len(score)) + score, 600, 35)
    ...
    def create_main_menu_labels(self):
        ...
        self.create_label(self.start_game, c.START_GAME, 370, 220)
        
        if self.game_info[c.SOUND_ON]:
            self.create_label(self.sound_option, 'SOUND -ON', 320, 270)
        else:
            self.create_label(self.sound_option, 'SOUND -OFF', 310, 270)
        if self.game_info[c.SFX_ON]:
            self.create_label(self.sfx_option, 'SFX -ON', 340, 320)
        else:
            self.create_label(self.sfx_option, 'SFX -OFF', 330, 320)
        self.create_label(self.exit, 'EXIT', 380, 370)
        self.state_labels = [self.start_game, self.sound_option, self.sfx_option, self.exit]
    ...
    def create_victory_labels(self):
        victory_label = []
        
        self.create_label(victory_label, 'VICTORY', 300, 300)
        
        self.state_labels = [victory_label, *self.info_labels]
    ...
    def update(self, level_info, level=None):
        self.level = level
        if self.state == c.MAIN_MENU:
            self.handle_main_state(level_info)
        self.handle_level_state(level_info)

    def handle_main_state(self, level_info):
        if level_info[c.SOUND_ON]:
            self.sound_option = []
            self.create_label(self.sound_option, 'SOUND -ON', 320, 270)
        else:
            self.sound_option = []
            self.create_label(self.sound_option, 'SOUND -OFF', 310, 270)
        if level_info[c.SFX_ON]:
            self.sfx_option = []
            self.create_label(self.sfx_option, 'SFX -ON', 340, 320)
        else:
            self.sfx_option = []
