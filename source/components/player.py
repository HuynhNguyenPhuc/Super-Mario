...
class Player(pg.sprite.Sprite):
    def __init__(self, player_name):
        ...
        self.has_jumped_sound_played = False
        self.is_powerup_sfx = False

    def restart(self):
        '''restart after player is dead or go to next level'''
        if self.dead:
            self.dead = False
            setup.SOUND.music_channel.play(setup.SOUND.soundtrack)
            self.big = False
            self.fire = False
    ...
    def jumping(self, keys, fire_group):
        ...
        setup.SOUND.play_sfx(setup.SOUND.jump)
        # self.has_jumped_sound_played = True
    ...
    def changing_to_big(self):
        if not self.is_powerup_sfx:
            setup.SOUND.play_sfx(setup.SOUND.powerup)
            self.is_powerup_sfx = True
    ...
    def start_death_jump(self, game_info):
        self.dead = True
        setup.SOUND.music_channel.stop()
        setup.SOUND.music_channel.play(setup.SOUND.death)
        self.y_vel = -11
        self.gravity = .5
        self.frame_index = 6
